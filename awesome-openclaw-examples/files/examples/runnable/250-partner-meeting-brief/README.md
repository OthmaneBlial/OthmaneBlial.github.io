...
- Collects partner history, current goals, and open commercial questions within the declared workflow scope.
- Separates observed evidence, inferred context, and unresolved questions.
- Produces a pre-meeting brief with evidence and questions to resolve.
...
export CRON_NAME="Partner Meeting Brief"
```

4. Check the local OpenClaw prerequisite:

```bash
bash examples/runnable/250-partner-meeting-brief/scripts/check_prereqs.sh
```

5. Read [the illustrative sample output](sample-output.md), then install the draft-only cron job:

```bash
bash examples/runnable/250-partner-meeting-brief/scripts/install_cron.sh
```
