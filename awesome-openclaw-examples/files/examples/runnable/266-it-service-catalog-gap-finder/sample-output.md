# Illustrative Sample Output - IT Service Catalog Gap Finder
...
- Signal: service requests, catalog entries, and routing outcomes produced 7 reviewable items in the selected scope.
...
- Review the evidence queue and confirm the owner for a gap queue for missing service descriptions and ownership.
- Keep the proposed output in draft state until the human reviewer approves the wording and scope.

## KPI snapshot

- repeat service requests with a proposed catalog answer: baseline pending from the first two runs.
