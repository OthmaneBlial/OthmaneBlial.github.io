# Illustrative Sample Output - Cross-timezone Handoff Pack
...
- Signal: handoff notes, open threads, and time-zone coverage produced 7 reviewable items in the selected scope.
...
- Review the evidence queue and confirm the owner for a handoff pack with context, explicit asks, and stop conditions.
- Keep the proposed output in draft state until the human reviewer approves the wording and scope.

## KPI snapshot

- handoffs accepted without re-explaining context: baseline pending from the first two runs.
