# 275 - Security Exception Expiry Watch
...
export CRON_NAME="Security Exception Expiry Watch"
```

4. Check the local OpenClaw prerequisite:

```bash
bash examples/runnable/275-security-exception-expiry-watch/scripts/check_prereqs.sh
```

5. Read [the illustrative sample output](sample-output.md), then install the draft-only cron job:

```bash
bash examples/runnable/275-security-exception-expiry-watch/scripts/install_cron.sh
```
