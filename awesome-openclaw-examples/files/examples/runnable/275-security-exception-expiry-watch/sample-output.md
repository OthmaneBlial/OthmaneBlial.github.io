# Illustrative Sample Output - Security Exception Expiry Watch
...
- Signal: approved security exceptions and expiry dates produced 7 reviewable items in the selected scope.
...
- Review the evidence queue and confirm the owner for an expiry queue that separates renewal evidence from closure work.
- Keep the proposed output in draft state until the human reviewer approves the wording and scope.

## KPI snapshot

- exceptions reviewed before expiry: baseline pending from the first two runs.
