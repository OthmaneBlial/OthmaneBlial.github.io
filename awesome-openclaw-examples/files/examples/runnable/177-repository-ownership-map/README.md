...
export CRON_NAME="Repository Ownership Map"
```

4. Check the local OpenClaw prerequisite:

```bash
bash examples/runnable/177-repository-ownership-map/scripts/check_prereqs.sh
```

5. Read [the illustrative sample output](sample-output.md), then install the draft-only cron job:

```bash
bash examples/runnable/177-repository-ownership-map/scripts/install_cron.sh
```
