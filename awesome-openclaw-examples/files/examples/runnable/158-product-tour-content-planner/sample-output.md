# Illustrative Sample Output - Product Tour Content Planner
...
- Signal: feature goals, help content, and onboarding questions produced 7 reviewable items in the selected scope.
...
- Review the evidence queue and confirm the owner for a product-tour outline with steps, proof points, and exit criteria.
- Keep the proposed output in draft state until the human reviewer approves the wording and scope.

## KPI snapshot

- tour steps with a measurable completion event: baseline pending from the first two runs.
