# Illustrative Sample Output - Service Dependency Map
...
- Signal: service manifests, repository links, and ownership records produced 7 reviewable items in the selected scope.
...
- Review the evidence queue and confirm the owner for a proposed dependency map with confidence and stale-edge flags.
- Keep the proposed output in draft state until the human reviewer approves the wording and scope.

## KPI snapshot

- services with a reviewed critical dependency path: baseline pending from the first two runs.
