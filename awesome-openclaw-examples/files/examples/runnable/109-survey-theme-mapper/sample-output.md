# Illustrative Sample Output - Survey Theme Mapper
...
- Signal: survey responses grouped by question and audience produced 7 reviewable items in the selected scope.
...
- Review the evidence queue and confirm the owner for a theme map with representative evidence and response counts.
- Keep the proposed output in draft state until the human reviewer approves the wording and scope.

## KPI snapshot

- manual coding hours avoided per survey: baseline pending from the first two runs.
