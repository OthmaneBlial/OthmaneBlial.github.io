# Illustrative Sample Output - Churn Save Playbook Brief
...
- Signal: churn signals, stated reasons, and account history produced 7 reviewable items in the selected scope.
...
- Review the evidence queue and confirm the owner for a save-plan brief that clearly marks hypotheses and approved options.
- Keep the proposed output in draft state until the human reviewer approves the wording and scope.

## KPI snapshot

- at-risk accounts with an owner and next review: baseline pending from the first two runs.
