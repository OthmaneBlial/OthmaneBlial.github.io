# Illustrative Sample Output - Forecast Assumption Register
...
- Signal: forecast inputs, owner comments, and prior-period variances produced 7 reviewable items in the selected scope.
...
- Review the evidence queue and confirm the owner for a dated assumption register with change reasons and confidence.
- Keep the proposed output in draft state until the human reviewer approves the wording and scope.

## KPI snapshot

- forecast assumptions reviewed before commit: baseline pending from the first two runs.
