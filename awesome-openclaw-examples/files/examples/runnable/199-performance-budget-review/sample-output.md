# Illustrative Sample Output - Performance Budget Review
...
- Signal: performance budgets, build artifacts, and recent changes produced 7 reviewable items in the selected scope.
...
- Review the evidence queue and confirm the owner for a budget review with evidence, regression candidates, and next checks.
- Keep the proposed output in draft state until the human reviewer approves the wording and scope.

## KPI snapshot

- budget violations reviewed before release: baseline pending from the first two runs.
