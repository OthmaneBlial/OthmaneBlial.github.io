# 194 - Database Migration Readiness

Database Migration Readiness turns migration plans, schema diffs, rollback notes, and dependencies into a readiness brief with missing rehearsal or rollback evidence. It is a bounded starter for per migration review, with human approval before any external write or outbound message.

## What It Does

- Collects migration plans, schema diffs, rollback notes, and dependencies within the declared workflow scope.
- Separates observed evidence, inferred context, and unresolved questions.
- Produces a readiness brief with missing rehearsal or rollback evidence.
...
export CRON_NAME="Database Migration Readiness"
```

4. Check the local OpenClaw prerequisite:

```bash
bash examples/runnable/194-database-migration-readiness/scripts/check_prereqs.sh
```

5. Read [the illustrative sample output](sample-output.md), then install the draft-only cron job:

```bash
bash examples/runnable/194-database-migration-readiness/scripts/install_cron.sh
```
...
- migrations passing preflight before approval.
...
- Treat schema and operational data as sensitive and minimize the source scope before the first run.
