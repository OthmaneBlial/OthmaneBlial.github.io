# Illustrative Sample Output - Database Migration Readiness
...
- Signal: migration plans, schema diffs, rollback notes, and dependencies produced 7 reviewable items in the selected scope.
...
- Review the evidence queue and confirm the owner for a readiness brief with missing rehearsal or rollback evidence.
- Keep the proposed output in draft state until the human reviewer approves the wording and scope.

## KPI snapshot

- migrations passing preflight before approval: baseline pending from the first two runs.
