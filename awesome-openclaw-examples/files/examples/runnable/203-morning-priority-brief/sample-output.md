# Illustrative Sample Output - Morning Priority Brief
...
- Signal: today's calendar, tasks, and explicitly marked priorities produced 7 reviewable items in the selected scope.
...
- Review the evidence queue and confirm the owner for a short brief that distinguishes fixed commitments from movable work.
- Keep the proposed output in draft state until the human reviewer approves the wording and scope.

## KPI snapshot

- time from start of day to a chosen first task: baseline pending from the first two runs.
