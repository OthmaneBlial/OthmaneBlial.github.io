# Illustrative Sample Output - Search Synonym Curator
...
- Signal: failed searches and the terms people actually use produced 7 reviewable items in the selected scope.
...
- Review the evidence queue and confirm the owner for a proposed synonym set with collision checks and review status.
- Keep the proposed output in draft state until the human reviewer approves the wording and scope.

## KPI snapshot

- zero-result searches reduced: baseline pending from the first two runs.
