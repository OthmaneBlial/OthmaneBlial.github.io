# Illustrative Sample Output - Third-Party Risk Renewal Pack
...
- Signal: vendor review documents, expiry dates, and prior findings produced 7 reviewable items in the selected scope.
...
- Review the evidence queue and confirm the owner for a renewal pack with evidence gaps and a decision checklist.
- Keep the proposed output in draft state until the human reviewer approves the wording and scope.

## KPI snapshot

- renewals reviewed before the vendor decision date: baseline pending from the first two runs.
