# Illustrative Sample Output - Support Queue Capacity Planner
...
- Signal: open support work, due dates, and available coverage produced 7 reviewable items in the selected scope.
...
- Review the evidence queue and confirm the owner for a capacity plan with overflow and escalation thresholds.
- Keep the proposed output in draft state until the human reviewer approves the wording and scope.

## KPI snapshot

- tickets assigned before SLA risk: baseline pending from the first two runs.
