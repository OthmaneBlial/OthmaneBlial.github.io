# Illustrative Sample Output - Assignment Feedback Theme Miner
...
- Signal: anonymized assignment feedback and rubric criteria produced 7 reviewable items in the selected scope.
...
- Review the evidence queue and confirm the owner for a theme report with examples and a revision queue for instructors.
- Keep the proposed output in draft state until the human reviewer approves the wording and scope.

## KPI snapshot

- feedback themes tied to a concrete course change: baseline pending from the first two runs.
