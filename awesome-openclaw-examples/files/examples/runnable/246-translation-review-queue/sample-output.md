# Illustrative Sample Output - Translation Review Queue
...
- Signal: source copy, translated copy, glossary, and target locale produced 7 reviewable items in the selected scope.
...
- Review the evidence queue and confirm the owner for a review queue for meaning drift, placeholders, and terminology.
- Keep the proposed output in draft state until the human reviewer approves the wording and scope.

## KPI snapshot

- translations approved with zero blocking terminology issues: baseline pending from the first two runs.
