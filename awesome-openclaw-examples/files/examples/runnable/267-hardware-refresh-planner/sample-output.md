# Illustrative Sample Output - Hardware Refresh Planner
...
- Signal: device age, warranty dates, user needs, and budget constraints produced 7 reviewable items in the selected scope.
...
- Review the evidence queue and confirm the owner for a refresh shortlist with assumptions and approval status.
- Keep the proposed output in draft state until the human reviewer approves the wording and scope.

## KPI snapshot

- devices reviewed before warranty or support expiry: baseline pending from the first two runs.
