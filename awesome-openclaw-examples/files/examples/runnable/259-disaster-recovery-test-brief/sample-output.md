# Illustrative Sample Output - Disaster Recovery Test Brief
...
- Signal: recovery test notes, objectives, and measured restore evidence produced 7 reviewable items in the selected scope.
...
- Review the evidence queue and confirm the owner for a test brief that separates observed recovery from untested assumptions.
- Keep the proposed output in draft state until the human reviewer approves the wording and scope.

## KPI snapshot

- recovery objectives with a dated test result: baseline pending from the first two runs.
