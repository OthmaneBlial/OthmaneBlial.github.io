# Illustrative Sample Output - Incident Lessons Tracker
...
- Signal: post-incident findings, corrective actions, and due dates produced 7 reviewable items in the selected scope.
...
- Review the evidence queue and confirm the owner for a lessons tracker with repeat patterns and action status.
- Keep the proposed output in draft state until the human reviewer approves the wording and scope.

## KPI snapshot

- lessons with completed or explicitly accepted actions: baseline pending from the first two runs.
