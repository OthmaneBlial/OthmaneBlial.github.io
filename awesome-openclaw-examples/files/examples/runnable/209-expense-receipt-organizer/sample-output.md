# Illustrative Sample Output - Expense Receipt Organizer
...
- Signal: receipts, transaction notes, and declared categories produced 7 reviewable items in the selected scope.
...
- Review the evidence queue and confirm the owner for a draft expense index with missing fields and duplicate candidates.
- Keep the proposed output in draft state until the human reviewer approves the wording and scope.

## KPI snapshot

- receipts categorized before the reporting deadline: baseline pending from the first two runs.
