# Illustrative Sample Output - Open Issues Triage Board
...
- Signal: open issues, labels, age, and recent activity produced 7 reviewable items in the selected scope.
...
- Review the evidence queue and confirm the owner for a triage board with evidence-backed priority and next owner.
- Keep the proposed output in draft state until the human reviewer approves the wording and scope.

## KPI snapshot

- issues triaged within the agreed queue SLA: baseline pending from the first two runs.
