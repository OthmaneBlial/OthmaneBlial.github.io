# Illustrative Sample Output - Policy Change Impact Map
...
- Signal: a policy revision and the teams, workflows, and systems it touches produced 7 reviewable items in the selected scope.
...
- Review the evidence queue and confirm the owner for an impact map with owners, open decisions, and communication needs.
- Keep the proposed output in draft state until the human reviewer approves the wording and scope.

## KPI snapshot

- affected owners identified before policy rollout: baseline pending from the first two runs.
