# Illustrative Sample Output - Medication Refill Reminder
...
- Signal: user-entered refill dates and pharmacy reminders produced 7 reviewable items in the selected scope.
...
- Review the evidence queue and confirm the owner for a reminder queue with a clear instruction to confirm with the provider or pharmacy.
- Keep the proposed output in draft state until the human reviewer approves the wording and scope.

## KPI snapshot

- refills requested before the user-defined buffer: baseline pending from the first two runs.
