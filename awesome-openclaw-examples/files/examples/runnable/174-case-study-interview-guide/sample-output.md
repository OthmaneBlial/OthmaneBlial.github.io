# Illustrative Sample Output - Case Study Interview Guide
...
- Signal: approved case-study goals, customer context, and existing proof produced 7 reviewable items in the selected scope.
...
- Review the evidence queue and confirm the owner for a non-leading interview guide with evidence gaps and consent reminders.
- Keep the proposed output in draft state until the human reviewer approves the wording and scope.

## KPI snapshot

- interviews completed with coverage of key proof points: baseline pending from the first two runs.
