# 174 - Case Study Interview Guide

Case Study Interview Guide turns approved case-study goals, customer context, and existing proof into a non-leading interview guide with evidence gaps and consent reminders. It is a bounded starter for per case study review, with human approval before any external write or outbound message.

## What It Does

- Collects approved case-study goals, customer context, and existing proof within the declared workflow scope.
- Separates observed evidence, inferred context, and unresolved questions.
- Produces a non-leading interview guide with evidence gaps and consent reminders.
...
export CRON_NAME="Case Study Interview Guide"
```

4. Check the local OpenClaw prerequisite:

```bash
bash examples/runnable/174-case-study-interview-guide/scripts/check_prereqs.sh
```

5. Read [the illustrative sample output](sample-output.md), then install the draft-only cron job:

```bash
bash examples/runnable/174-case-study-interview-guide/scripts/install_cron.sh
```
...
- interviews completed with coverage of key proof points.
...
- Treat customer identity and confidential outcomes as sensitive and minimize the source scope before the first run.
