# Illustrative Sample Output - Async Standup Digest
...
- Signal: async standups and carry-over items produced 7 reviewable items in the selected scope.
...
- Review the evidence queue and confirm the owner for a compact digest that distinguishes progress from unverified status.
- Keep the proposed output in draft state until the human reviewer approves the wording and scope.

## KPI snapshot

- standups read before the first coordination meeting: baseline pending from the first two runs.
