# Illustrative Sample Output - Reading List Annotation Digest
...
- Signal: reading notes, highlights, and a declared research question produced 7 reviewable items in the selected scope.
...
- Review the evidence queue and confirm the owner for an annotation digest that keeps quotations linked to their source.
- Keep the proposed output in draft state until the human reviewer approves the wording and scope.

## KPI snapshot

- research notes with source references: baseline pending from the first two runs.
