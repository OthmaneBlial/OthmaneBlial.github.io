# Illustrative Sample Output - Customer Advisory Board Agenda
...
- Signal: member priorities, prior actions, and discussion candidates produced 7 reviewable items in the selected scope.
...
- Review the evidence queue and confirm the owner for an agenda balanced across member needs and product questions.
- Keep the proposed output in draft state until the human reviewer approves the wording and scope.

## KPI snapshot

- agenda items with a clear decision or learning goal: baseline pending from the first two runs.
