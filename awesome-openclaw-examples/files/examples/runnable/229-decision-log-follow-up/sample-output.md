# Illustrative Sample Output - Decision Log Follow-up
...
- Signal: decision records, due dates, and linked action items produced 7 reviewable items in the selected scope.
...
- Review the evidence queue and confirm the owner for a follow-up queue for decisions without an owner or review date.
- Keep the proposed output in draft state until the human reviewer approves the wording and scope.

## KPI snapshot

- decisions with a current next action: baseline pending from the first two runs.
