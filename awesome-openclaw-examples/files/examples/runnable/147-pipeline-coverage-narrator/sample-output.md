# Illustrative Sample Output - Pipeline Coverage Narrator
...
- Signal: pipeline stages, forecast categories, and target coverage produced 7 reviewable items in the selected scope.
...
- Review the evidence queue and confirm the owner for an evidence-linked coverage narrative with scenario boundaries.
- Keep the proposed output in draft state until the human reviewer approves the wording and scope.

## KPI snapshot

- forecast review time: baseline pending from the first two runs.
