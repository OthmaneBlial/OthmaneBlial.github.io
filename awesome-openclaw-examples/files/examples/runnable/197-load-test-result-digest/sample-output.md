# Illustrative Sample Output - Load Test Result Digest
...
- Signal: load-test runs, thresholds, and environment notes produced 7 reviewable items in the selected scope.
...
- Review the evidence queue and confirm the owner for a digest separating measured results from extrapolation.
- Keep the proposed output in draft state until the human reviewer approves the wording and scope.

## KPI snapshot

- load-test findings reviewed before capacity decisions: baseline pending from the first two runs.
