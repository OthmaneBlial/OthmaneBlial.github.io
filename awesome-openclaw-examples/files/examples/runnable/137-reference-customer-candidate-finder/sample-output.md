# Illustrative Sample Output - Reference Customer Candidate Finder
...
- Signal: customer outcomes, consent status, and relationship notes produced 7 reviewable items in the selected scope.
...
- Review the evidence queue and confirm the owner for a shortlist with evidence and a human approval gate before outreach.
- Keep the proposed output in draft state until the human reviewer approves the wording and scope.

## KPI snapshot

- approved reference candidates per quarter: baseline pending from the first two runs.
