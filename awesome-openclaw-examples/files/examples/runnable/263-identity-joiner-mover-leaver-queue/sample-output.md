# Illustrative Sample Output - Identity Joiner Mover Leaver Queue
...
- Signal: approved personnel events and account checklist state produced 7 reviewable items in the selected scope.
...
- Review the evidence queue and confirm the owner for a lifecycle queue with evidence and a manual completion gate.
- Keep the proposed output in draft state until the human reviewer approves the wording and scope.

## KPI snapshot

- lifecycle events completed by their target date: baseline pending from the first two runs.
