# Illustrative Sample Output - Win-Loss Interview Scheduler
...
- Signal: closed-deal context, consent status, and interviewer availability produced 7 reviewable items in the selected scope.
...
- Review the evidence queue and confirm the owner for a proposed interview queue with conflict-safe time options.
- Keep the proposed output in draft state until the human reviewer approves the wording and scope.

## KPI snapshot

- interviews booked within the target window: baseline pending from the first two runs.
