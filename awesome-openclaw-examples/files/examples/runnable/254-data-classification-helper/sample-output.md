# Illustrative Sample Output - Data Classification Helper
...
- Signal: document samples and the organization's classification rubric produced 7 reviewable items in the selected scope.
...
- Review the evidence queue and confirm the owner for a draft classification recommendation with evidence and reviewer sign-off.
- Keep the proposed output in draft state until the human reviewer approves the wording and scope.

## KPI snapshot

- documents receiving a reviewed classification: baseline pending from the first two runs.
