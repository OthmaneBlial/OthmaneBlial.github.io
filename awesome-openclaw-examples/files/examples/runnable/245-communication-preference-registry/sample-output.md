# Illustrative Sample Output - Communication Preference Registry
...
- Signal: explicit communication preferences and source dates produced 7 reviewable items in the selected scope.
...
- Review the evidence queue and confirm the owner for a reviewable registry with conflicting or stale preferences flagged.
- Keep the proposed output in draft state until the human reviewer approves the wording and scope.

## KPI snapshot

- records with an up-to-date preference source: baseline pending from the first two runs.
