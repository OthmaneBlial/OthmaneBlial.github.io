# Illustrative Sample Output - Lead Response Time Watch
...
- Signal: new lead timestamps, routing outcomes, and response events produced 7 reviewable items in the selected scope.
...
- Review the evidence queue and confirm the owner for a response-time digest with aging leads and routing gaps.
- Keep the proposed output in draft state until the human reviewer approves the wording and scope.

## KPI snapshot

- median time to first reviewed response: baseline pending from the first two runs.
