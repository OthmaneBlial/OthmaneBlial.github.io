# Illustrative Sample Output - Sales Territory Handoff
...
- Signal: account ownership changes, history, and next commitments produced 7 reviewable items in the selected scope.
...
- Review the evidence queue and confirm the owner for a handoff brief with explicit ownership and customer-safe next steps.
- Keep the proposed output in draft state until the human reviewer approves the wording and scope.

## KPI snapshot

- handoffs accepted by the new owner: baseline pending from the first two runs.
