# Illustrative Sample Output - Forecast Commit Evidence Pack
...
- Signal: commit opportunities, next steps, and dated customer signals produced 7 reviewable items in the selected scope.
...
- Review the evidence queue and confirm the owner for a commit review pack that flags evidence gaps without changing CRM data.
- Keep the proposed output in draft state until the human reviewer approves the wording and scope.

## KPI snapshot

- commits with a recent supporting signal: baseline pending from the first two runs.
