# Illustrative Sample Output - API Contract Drift Watch
...
- Signal: versioned API schemas, changelogs, and consumer notes produced 7 reviewable items in the selected scope.
...
- Review the evidence queue and confirm the owner for a drift report with breaking-change candidates and evidence links.
- Keep the proposed output in draft state until the human reviewer approves the wording and scope.

## KPI snapshot

- contract changes reviewed before consumer rollout: baseline pending from the first two runs.
