# Illustrative Sample Output - Infrastructure Cost Anomaly Brief
...
- Signal: approved infrastructure cost exports and budget thresholds produced 7 reviewable items in the selected scope.
...
- Review the evidence queue and confirm the owner for a cost anomaly brief with evidence and an owner for investigation.
- Keep the proposed output in draft state until the human reviewer approves the wording and scope.

## KPI snapshot

- anomalies acknowledged before the next billing cycle: baseline pending from the first two runs.
