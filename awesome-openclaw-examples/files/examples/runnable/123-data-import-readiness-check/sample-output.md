# Illustrative Sample Output - Data Import Readiness Check
...
- Signal: a proposed import file, schema, and acceptance rules produced 7 reviewable items in the selected scope.
...
- Review the evidence queue and confirm the owner for a preflight report with row-level samples and blocking issues.
- Keep the proposed output in draft state until the human reviewer approves the wording and scope.

## KPI snapshot

- imports passing preflight on first retry: baseline pending from the first two runs.
