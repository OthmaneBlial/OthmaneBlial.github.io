# 106 - Database Query Review Desk

Database Query Review Desk turns proposed analytical queries and their stated purpose into a review note covering assumptions, joins, filters, and test cases. It is a bounded starter for on demand review, with human approval before any external write or outbound message.

## What It Does

- Collects proposed analytical queries and their stated purpose within the declared workflow scope.
- Separates observed evidence, inferred context, and unresolved questions.
- Produces a review note covering assumptions, joins, filters, and test cases.
...
export CRON_NAME="Database Query Review Desk"
```

4. Check the local OpenClaw prerequisite:

```bash
bash examples/runnable/106-database-query-review-desk/scripts/check_prereqs.sh
```

5. Read [the illustrative sample output](sample-output.md), then install the draft-only cron job:

```bash
bash examples/runnable/106-database-query-review-desk/scripts/install_cron.sh
```
...
- queries reviewed before shared use.
...
- Treat database schema and query contents as sensitive and minimize the source scope before the first run.
