# Illustrative Sample Output - Renewal Notice Draft Queue
...
- Signal: renewal dates, account context, and approved notice language produced 7 reviewable items in the selected scope.
...
- Review the evidence queue and confirm the owner for a draft-only queue with timing, rationale, and human send approval.
- Keep the proposed output in draft state until the human reviewer approves the wording and scope.

## KPI snapshot

- renewals entering review before notice date: baseline pending from the first two runs.
