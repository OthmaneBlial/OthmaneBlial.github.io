# Illustrative Sample Output - Security Training Follow-up
...
- Signal: training assignments, completion events, and exception notes produced 7 reviewable items in the selected scope.
...
- Review the evidence queue and confirm the owner for a follow-up queue that distinguishes reminders from escalations.
- Keep the proposed output in draft state until the human reviewer approves the wording and scope.

## KPI snapshot

- required training completed before the policy deadline: baseline pending from the first two runs.
