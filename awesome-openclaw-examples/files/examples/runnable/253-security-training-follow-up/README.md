# 253 - Security Training Follow-up

Security Training Follow-up turns training assignments, completion events, and exception notes into a follow-up queue that distinguishes reminders from escalations. It is a bounded starter for weekly review, with human approval before any external write or outbound message.

## What It Does

- Collects training assignments, completion events, and exception notes within the declared workflow scope.
- Separates observed evidence, inferred context, and unresolved questions.
- Produces a follow-up queue that distinguishes reminders from escalations.
...
export CRON_NAME="Security Training Follow-up"
```

4. Check the local OpenClaw prerequisite:

```bash
bash examples/runnable/253-security-training-follow-up/scripts/check_prereqs.sh
```

5. Read [the illustrative sample output](sample-output.md), then install the draft-only cron job:

```bash
bash examples/runnable/253-security-training-follow-up/scripts/install_cron.sh
```
...
- required training completed before the policy deadline.
...
- Treat employee identity and security training status as sensitive and minimize the source scope before the first run.
