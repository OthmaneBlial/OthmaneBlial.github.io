# Illustrative Sample Output - Business Continuity Exercise Pack
...
- Signal: continuity plans, exercise goals, and system dependencies produced 7 reviewable items in the selected scope.
...
- Review the evidence queue and confirm the owner for a tabletop exercise pack with injects, observers, and capture prompts.
- Keep the proposed output in draft state until the human reviewer approves the wording and scope.

## KPI snapshot

- exercise actions assigned before the after-action review: baseline pending from the first two runs.
