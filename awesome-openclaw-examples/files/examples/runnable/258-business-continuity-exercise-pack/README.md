# 258 - Business Continuity Exercise Pack

Business Continuity Exercise Pack turns continuity plans, exercise goals, and system dependencies into a tabletop exercise pack with injects, observers, and capture prompts. It is a bounded starter for quarterly review, with human approval before any external write or outbound message.

## What It Does

- Collects continuity plans, exercise goals, and system dependencies within the declared workflow scope.
- Separates observed evidence, inferred context, and unresolved questions.
- Produces a tabletop exercise pack with injects, observers, and capture prompts.
...
export CRON_NAME="Business Continuity Exercise Pack"
```

4. Check the local OpenClaw prerequisite:

```bash
bash examples/runnable/258-business-continuity-exercise-pack/scripts/check_prereqs.sh
```

5. Read [the illustrative sample output](sample-output.md), then install the draft-only cron job:

```bash
bash examples/runnable/258-business-continuity-exercise-pack/scripts/install_cron.sh
```
...
- exercise actions assigned before the after-action review.
...
- Treat business continuity plans and system dependencies as sensitive and minimize the source scope before the first run.
