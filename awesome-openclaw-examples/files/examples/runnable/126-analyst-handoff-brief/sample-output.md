# Illustrative Sample Output - Analyst Handoff Brief
...
- Signal: an analyst's working notes, sources, and unresolved questions produced 7 reviewable items in the selected scope.
...
- Review the evidence queue and confirm the owner for a compact handoff with evidence links and an explicit next owner.
- Keep the proposed output in draft state until the human reviewer approves the wording and scope.

## KPI snapshot

- handoffs accepted without clarification loops: baseline pending from the first two runs.
