# Illustrative Sample Output - Family Calendar Conflict Resolver
...
- Signal: shared calendar events, constraints, and stated priorities produced 7 reviewable items in the selected scope.
...
- Review the evidence queue and confirm the owner for a conflict report with proposed options for human selection.
- Keep the proposed output in draft state until the human reviewer approves the wording and scope.

## KPI snapshot

- conflicts resolved before the next planning checkpoint: baseline pending from the first two runs.
