# Illustrative Sample Output - Team Weekly Check-in Synthesizer
...
- Signal: team check-ins, blockers, and requested support produced 7 reviewable items in the selected scope.
...
- Review the evidence queue and confirm the owner for a factual synthesis grouped by progress, blocker, and follow-up owner.
- Keep the proposed output in draft state until the human reviewer approves the wording and scope.

## KPI snapshot

- check-ins with an explicit follow-up state: baseline pending from the first two runs.
