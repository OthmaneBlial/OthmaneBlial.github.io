# Illustrative Sample Output - Phishing Report Triage
...
- Signal: reported messages and safe-analysis guidance produced 7 reviewable items in the selected scope.
...
- Review the evidence queue and confirm the owner for a triage digest with indicators, user guidance, and escalation state.
- Keep the proposed output in draft state until the human reviewer approves the wording and scope.

## KPI snapshot

- reports triaged within the response target: baseline pending from the first two runs.
