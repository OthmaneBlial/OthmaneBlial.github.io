# Illustrative Sample Output - Community Moderator Handoff
...
- Signal: moderation events, unresolved reports, and policy references produced 7 reviewable items in the selected scope.
...
- Review the evidence queue and confirm the owner for a shift handoff with evidence links and explicit escalation thresholds.
- Keep the proposed output in draft state until the human reviewer approves the wording and scope.

## KPI snapshot

- open reports with a current next owner: baseline pending from the first two runs.
