# Illustrative Sample Output - Packing List Builder
...
- Signal: trip duration, activities, weather, and existing packing templates produced 7 reviewable items in the selected scope.
...
- Review the evidence queue and confirm the owner for a categorized draft list with assumptions clearly marked.
- Keep the proposed output in draft state until the human reviewer approves the wording and scope.

## KPI snapshot

- packing omissions found during pre-departure review: baseline pending from the first two runs.
