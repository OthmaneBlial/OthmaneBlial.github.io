# Illustrative Sample Output - Newsletter Issue Planner
...
- Signal: approved stories, audience segments, and prior issue performance produced 7 reviewable items in the selected scope.
...
- Review the evidence queue and confirm the owner for a draft issue plan with ordering, source links, and open approvals.
- Keep the proposed output in draft state until the human reviewer approves the wording and scope.

## KPI snapshot

- issue plans approved before the writing window: baseline pending from the first two runs.
