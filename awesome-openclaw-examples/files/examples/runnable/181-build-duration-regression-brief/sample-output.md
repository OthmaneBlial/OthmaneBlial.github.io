# Illustrative Sample Output - Build Duration Regression Brief
...
- Signal: build duration history, workflow changes, and target budgets produced 7 reviewable items in the selected scope.
...
- Review the evidence queue and confirm the owner for a regression brief with likely contributors and a validation plan.
- Keep the proposed output in draft state until the human reviewer approves the wording and scope.

## KPI snapshot

- time to identify a build regression: baseline pending from the first two runs.
