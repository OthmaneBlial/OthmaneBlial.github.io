# Illustrative Sample Output - Beta Cohort Pulse
...
- Signal: beta usage snapshots, feedback, and reported issues produced 7 reviewable items in the selected scope.
...
- Review the evidence queue and confirm the owner for a cohort pulse that separates adoption, friction, and qualitative feedback.
- Keep the proposed output in draft state until the human reviewer approves the wording and scope.

## KPI snapshot

- beta participants with a current next action: baseline pending from the first two runs.
