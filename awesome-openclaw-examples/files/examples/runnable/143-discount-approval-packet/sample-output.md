# Illustrative Sample Output - Discount Approval Packet
...
- Signal: discount rationale, deal context, and approval limits produced 7 reviewable items in the selected scope.
...
- Review the evidence queue and confirm the owner for a review packet that calculates stated impact without approving anything.
- Keep the proposed output in draft state until the human reviewer approves the wording and scope.

## KPI snapshot

- approval packets complete on first review: baseline pending from the first two runs.
