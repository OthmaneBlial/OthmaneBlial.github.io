# Illustrative Sample Output - Runbook Coverage Finder
...
- Signal: service inventory, incident history, and runbooks produced 7 reviewable items in the selected scope.
...
- Review the evidence queue and confirm the owner for a gap queue showing services without a current recovery path.
- Keep the proposed output in draft state until the human reviewer approves the wording and scope.

## KPI snapshot

- critical services with a reviewed runbook: baseline pending from the first two runs.
