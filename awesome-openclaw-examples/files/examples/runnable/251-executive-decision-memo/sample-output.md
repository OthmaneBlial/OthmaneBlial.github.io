# Illustrative Sample Output - Executive Decision Memo
...
- Signal: decision context, options, evidence, and constraints produced 7 reviewable items in the selected scope.
...
- Review the evidence queue and confirm the owner for a concise decision memo that makes assumptions and approval state explicit.
- Keep the proposed output in draft state until the human reviewer approves the wording and scope.

## KPI snapshot

- decisions reached with a cited evidence packet: baseline pending from the first two runs.
