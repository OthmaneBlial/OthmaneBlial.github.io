# Illustrative Sample Output - Action Item Aging Radar
...
- Signal: open action items, owners, due dates, and last updates produced 7 reviewable items in the selected scope.
...
- Review the evidence queue and confirm the owner for an aging view with suggested escalation language in draft state.
- Keep the proposed output in draft state until the human reviewer approves the wording and scope.

## KPI snapshot

- overdue actions receiving a reviewed next step: baseline pending from the first two runs.
