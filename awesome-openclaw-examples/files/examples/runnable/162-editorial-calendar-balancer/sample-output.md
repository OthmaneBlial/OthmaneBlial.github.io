# Illustrative Sample Output - Editorial Calendar Balancer
...
- Signal: planned pieces, audiences, channels, and publishing constraints produced 7 reviewable items in the selected scope.
...
- Review the evidence queue and confirm the owner for a calendar review showing topic gaps, collisions, and workload risk.
- Keep the proposed output in draft state until the human reviewer approves the wording and scope.

## KPI snapshot

- calendar conflicts found before the next planning cycle: baseline pending from the first two runs.
