# Illustrative Sample Output - Open Source Maintainer Digest
...
- Signal: issues, pull requests, releases, and maintainer notes produced 7 reviewable items in the selected scope.
...
- Review the evidence queue and confirm the owner for a digest of contributor workload, release risk, and decisions needed.
- Keep the proposed output in draft state until the human reviewer approves the wording and scope.

## KPI snapshot

- maintainer actions with a current owner: baseline pending from the first two runs.
