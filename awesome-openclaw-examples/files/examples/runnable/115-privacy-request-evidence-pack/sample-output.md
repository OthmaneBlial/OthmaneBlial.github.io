# Illustrative Sample Output - Privacy Request Evidence Pack
...
- Signal: a scoped privacy request and matching source records produced 7 reviewable items in the selected scope.
...
- Review the evidence queue and confirm the owner for a review-ready evidence pack with gaps and a human approval checkpoint.
- Keep the proposed output in draft state until the human reviewer approves the wording and scope.

## KPI snapshot

- time to assemble a complete evidence set: baseline pending from the first two runs.
