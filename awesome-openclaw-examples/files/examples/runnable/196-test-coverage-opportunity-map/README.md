# 196 - Test Coverage Opportunity Map
...
export CRON_NAME="Test Coverage Opportunity Map"
```

4. Check the local OpenClaw prerequisite:

```bash
bash examples/runnable/196-test-coverage-opportunity-map/scripts/check_prereqs.sh
```

5. Read [the illustrative sample output](sample-output.md), then install the draft-only cron job:

```bash
bash examples/runnable/196-test-coverage-opportunity-map/scripts/install_cron.sh
```
