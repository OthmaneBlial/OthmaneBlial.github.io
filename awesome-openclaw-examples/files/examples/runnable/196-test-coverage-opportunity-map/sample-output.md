# Illustrative Sample Output - Test Coverage Opportunity Map
...
- Signal: coverage reports, changed files, and incident history produced 7 reviewable items in the selected scope.
...
- Review the evidence queue and confirm the owner for a prioritized opportunity map with suggested test boundaries.
- Keep the proposed output in draft state until the human reviewer approves the wording and scope.

## KPI snapshot

- high-risk gaps receiving a concrete test issue: baseline pending from the first two runs.
