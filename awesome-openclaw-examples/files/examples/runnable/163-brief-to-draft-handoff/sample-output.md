# Illustrative Sample Output - Brief-to-Draft Handoff
...
- Signal: an approved brief and the writer's open questions produced 7 reviewable items in the selected scope.
...
- Review the evidence queue and confirm the owner for a handoff packet with acceptance criteria and source links.
- Keep the proposed output in draft state until the human reviewer approves the wording and scope.

## KPI snapshot

- drafts started with no missing brief inputs: baseline pending from the first two runs.
