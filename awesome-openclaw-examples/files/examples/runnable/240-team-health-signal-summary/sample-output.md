# Illustrative Sample Output - Team Health Signal Summary
...
- Signal: voluntary check-ins, workload signals, and open support requests produced 7 reviewable items in the selected scope.
...
- Review the evidence queue and confirm the owner for a privacy-conscious summary that avoids individual diagnosis.
- Keep the proposed output in draft state until the human reviewer approves the wording and scope.

## KPI snapshot

- team-level signals reviewed with a named owner: baseline pending from the first two runs.
