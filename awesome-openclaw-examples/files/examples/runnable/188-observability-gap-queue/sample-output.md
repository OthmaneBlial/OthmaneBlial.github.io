# Illustrative Sample Output - Observability Gap Queue
...
- Signal: service inventory, alert definitions, and incident findings produced 7 reviewable items in the selected scope.
...
- Review the evidence queue and confirm the owner for a queue of missing signals with a concrete validation request.
- Keep the proposed output in draft state until the human reviewer approves the wording and scope.

## KPI snapshot

- gaps assigned before the next reliability review: baseline pending from the first two runs.
