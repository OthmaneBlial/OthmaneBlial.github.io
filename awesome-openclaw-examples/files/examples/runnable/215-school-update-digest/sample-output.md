# Illustrative Sample Output - School Update Digest
...
- Signal: school notices, deadlines, and family task assignments produced 7 reviewable items in the selected scope.
...
- Review the evidence queue and confirm the owner for a dated digest with action items and missing-information questions.
- Keep the proposed output in draft state until the human reviewer approves the wording and scope.

## KPI snapshot

- school deadlines captured before they become overdue: baseline pending from the first two runs.
