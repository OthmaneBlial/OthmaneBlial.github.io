# Illustrative Sample Output - Shared Inbox Assignment Board
...
- Signal: unassigned shared inbox threads and routing rules produced 7 reviewable items in the selected scope.
...
- Review the evidence queue and confirm the owner for an assignment queue with confidence, owner, and escalation reason.
- Keep the proposed output in draft state until the human reviewer approves the wording and scope.

## KPI snapshot

- threads assigned before their service threshold: baseline pending from the first two runs.
