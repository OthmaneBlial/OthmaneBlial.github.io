# Illustrative Sample Output - Dashboard Change Digest
...
- Signal: dashboard revisions and linked metric definitions produced 7 reviewable items in the selected scope.
...
- Review the evidence queue and confirm the owner for a change digest that calls out renamed, removed, and reinterpreted metrics.
- Keep the proposed output in draft state until the human reviewer approves the wording and scope.

## KPI snapshot

- stakeholders notified before a dashboard review: baseline pending from the first two runs.
