# Illustrative Sample Output - Security Advisory Impact Check
...
- Signal: dependency advisories and repository dependency manifests produced 7 reviewable items in the selected scope.
...
- Review the evidence queue and confirm the owner for an impact check with affected paths and a remediation review queue.
- Keep the proposed output in draft state until the human reviewer approves the wording and scope.

## KPI snapshot

- advisories with a confirmed impact status: baseline pending from the first two runs.
