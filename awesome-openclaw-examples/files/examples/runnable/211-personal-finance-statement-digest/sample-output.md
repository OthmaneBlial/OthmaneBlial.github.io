# Illustrative Sample Output - Personal Finance Statement Digest
...
- Signal: periodic statements and the owner's category rules produced 7 reviewable items in the selected scope.
...
- Review the evidence queue and confirm the owner for a categorized digest that flags unknown or unusual entries for review.
- Keep the proposed output in draft state until the human reviewer approves the wording and scope.

## KPI snapshot

- unknown transactions resolved before monthly close: baseline pending from the first two runs.
