# Illustrative Sample Output - SLA Breach Root Cause Brief
...
- Signal: breached support cases and their timeline events produced 7 reviewable items in the selected scope.
...
- Review the evidence queue and confirm the owner for a root-cause brief separating process, product, and staffing factors.
- Keep the proposed output in draft state until the human reviewer approves the wording and scope.

## KPI snapshot

- breaches with a recorded contributing factor: baseline pending from the first two runs.
