# 116 - Knowledge Graph Link Curator

Knowledge Graph Link Curator turns related documents, projects, people, and decisions into a small set of proposed links with reasons and source references. It is a bounded starter for weekly review, with human approval before any external write or outbound message.

## What It Does

- Collects related documents, projects, people, and decisions within the declared workflow scope.
- Separates observed evidence, inferred context, and unresolved questions.
- Produces a small set of proposed links with reasons and source references.
...
export CRON_NAME="Knowledge Graph Link Curator"
```

4. Check the local OpenClaw prerequisite:

```bash
bash examples/runnable/116-knowledge-graph-link-curator/scripts/check_prereqs.sh
```

5. Read [the illustrative sample output](sample-output.md), then install the draft-only cron job:

```bash
bash examples/runnable/116-knowledge-graph-link-curator/scripts/install_cron.sh
```
...
- accepted links per curation session.
...
- Treat private organizational knowledge as sensitive and minimize the source scope before the first run.
