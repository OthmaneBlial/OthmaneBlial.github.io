# 138 - Case Study Evidence Pack

Case Study Evidence Pack turns customer results, dated source notes, and approved quotes into a fact-checked case-study evidence pack with claim provenance. It is a bounded starter for per case study review, with human approval before any external write or outbound message.

## What It Does

- Collects customer results, dated source notes, and approved quotes within the declared workflow scope.
- Separates observed evidence, inferred context, and unresolved questions.
- Produces a fact-checked case-study evidence pack with claim provenance.
...
export CRON_NAME="Case Study Evidence Pack"
```

4. Check the local OpenClaw prerequisite:

```bash
bash examples/runnable/138-case-study-evidence-pack/scripts/check_prereqs.sh
```

5. Read [the illustrative sample output](sample-output.md), then install the draft-only cron job:

```bash
bash examples/runnable/138-case-study-evidence-pack/scripts/install_cron.sh
```
...
- draft claims with a source and approver.
...
- Treat customer results and quotes as sensitive and minimize the source scope before the first run.
