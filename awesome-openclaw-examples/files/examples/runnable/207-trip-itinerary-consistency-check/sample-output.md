# Illustrative Sample Output - Trip Itinerary Consistency Check
...
- Signal: reservations, calendar events, and itinerary notes produced 7 reviewable items in the selected scope.
...
- Review the evidence queue and confirm the owner for a conflict report for dates, time zones, transfers, and missing confirmations.
- Keep the proposed output in draft state until the human reviewer approves the wording and scope.

## KPI snapshot

- itinerary contradictions caught before departure: baseline pending from the first two runs.
