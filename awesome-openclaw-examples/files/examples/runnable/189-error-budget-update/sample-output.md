# Illustrative Sample Output - Error Budget Update
...
- Signal: approved SLO windows, incidents, and availability snapshots produced 7 reviewable items in the selected scope.
...
- Review the evidence queue and confirm the owner for an error-budget update with calculation inputs and caveats.
- Keep the proposed output in draft state until the human reviewer approves the wording and scope.

## KPI snapshot

- error-budget reviews with traceable inputs: baseline pending from the first two runs.
