# Illustrative Sample Output - Experiment Results Interpreter
...
- Signal: experiment plans, results, and pre-registered success criteria produced 7 reviewable items in the selected scope.
...
- Review the evidence queue and confirm the owner for a decision memo that distinguishes observed lift from uncertainty.
- Keep the proposed output in draft state until the human reviewer approves the wording and scope.

## KPI snapshot

- time from experiment close to decision: baseline pending from the first two runs.
