# 108 - Experiment Results Interpreter

Experiment Results Interpreter turns experiment plans, results, and pre-registered success criteria into a decision memo that distinguishes observed lift from uncertainty. It is a bounded starter for per experiment review, with human approval before any external write or outbound message.

## What It Does

- Collects experiment plans, results, and pre-registered success criteria within the declared workflow scope.
- Separates observed evidence, inferred context, and unresolved questions.
- Produces a decision memo that distinguishes observed lift from uncertainty.
...
export CRON_NAME="Experiment Results Interpreter"
```

4. Check the local OpenClaw prerequisite:

```bash
bash examples/runnable/108-experiment-results-interpreter/scripts/check_prereqs.sh
```

5. Read [the illustrative sample output](sample-output.md), then install the draft-only cron job:

```bash
bash examples/runnable/108-experiment-results-interpreter/scripts/install_cron.sh
```
...
- time from experiment close to decision.
...
- Treat user behavior and experiment cohorts as sensitive and minimize the source scope before the first run.
