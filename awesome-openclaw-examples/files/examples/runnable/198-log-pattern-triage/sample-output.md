# Illustrative Sample Output - Log Pattern Triage
...
- Signal: approved log samples and incident query windows produced 7 reviewable items in the selected scope.
...
- Review the evidence queue and confirm the owner for a pattern digest with redaction checks and suggested follow-up.
- Keep the proposed output in draft state until the human reviewer approves the wording and scope.

## KPI snapshot

- recurring error patterns with an owner: baseline pending from the first two runs.
