# Illustrative Sample Output - Duplicate Record Detector
...
- Signal: exported records with stable identifiers and match fields produced 7 reviewable items in the selected scope.
...
- Review the evidence queue and confirm the owner for a ranked merge-review queue that never mutates records automatically.
- Keep the proposed output in draft state until the human reviewer approves the wording and scope.

## KPI snapshot

- confirmed duplicate pairs per review hour: baseline pending from the first two runs.
