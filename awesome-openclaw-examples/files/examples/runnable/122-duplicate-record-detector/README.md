# 122 - Duplicate Record Detector

Duplicate Record Detector turns exported records with stable identifiers and match fields into a ranked merge-review queue that never mutates records automatically. It is a bounded starter for daily review, with human approval before any external write or outbound message.

## What It Does

- Collects exported records with stable identifiers and match fields within the declared workflow scope.
- Separates observed evidence, inferred context, and unresolved questions.
- Produces a ranked merge-review queue that never mutates records automatically.
...
export CRON_NAME="Duplicate Record Detector"
```

4. Check the local OpenClaw prerequisite:

```bash
bash examples/runnable/122-duplicate-record-detector/scripts/check_prereqs.sh
```

5. Read [the illustrative sample output](sample-output.md), then install the draft-only cron job:

```bash
bash examples/runnable/122-duplicate-record-detector/scripts/install_cron.sh
```
...
- confirmed duplicate pairs per review hour.
...
- Treat customer, vendor, or employee records as sensitive and minimize the source scope before the first run.
