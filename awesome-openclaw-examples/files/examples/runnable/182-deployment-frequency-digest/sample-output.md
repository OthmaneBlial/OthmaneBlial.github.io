# Illustrative Sample Output - Deployment Frequency Digest
...
- Signal: deployment records, rollback events, and release ownership produced 7 reviewable items in the selected scope.
...
- Review the evidence queue and confirm the owner for a factual delivery digest with gaps and comparison to the prior period.
- Keep the proposed output in draft state until the human reviewer approves the wording and scope.

## KPI snapshot

- deployments with a recorded outcome: baseline pending from the first two runs.
