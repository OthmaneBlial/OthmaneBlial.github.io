# Illustrative Sample Output - Roadmap Dependency Brief
...
- Signal: roadmap items, open dependencies, and delivery assumptions produced 7 reviewable items in the selected scope.
...
- Review the evidence queue and confirm the owner for a dependency brief with blockers, owners, and the next decision date.
- Keep the proposed output in draft state until the human reviewer approves the wording and scope.

## KPI snapshot

- roadmap items with a reviewed dependency status: baseline pending from the first two runs.
