# Illustrative Sample Output - Campaign UTM Hygiene Check
...
- Signal: campaign links, naming rules, and destination URLs produced 7 reviewable items in the selected scope.
...
- Review the evidence queue and confirm the owner for a preflight report for malformed parameters and inconsistent naming.
- Keep the proposed output in draft state until the human reviewer approves the wording and scope.

## KPI snapshot

- campaign links passing review before launch: baseline pending from the first two runs.
