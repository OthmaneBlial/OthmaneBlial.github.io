# Illustrative Sample Output - Event Follow-up Signal Digest
...
- Signal: event notes, opted-in contacts, and follow-up status produced 7 reviewable items in the selected scope.
...
- Review the evidence queue and confirm the owner for a draft-only follow-up queue with reasons and approval state.
- Keep the proposed output in draft state until the human reviewer approves the wording and scope.

## KPI snapshot

- follow-ups reviewed within the event SLA: baseline pending from the first two runs.
