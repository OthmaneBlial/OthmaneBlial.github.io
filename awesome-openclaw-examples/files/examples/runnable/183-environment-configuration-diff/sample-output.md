# Illustrative Sample Output - Environment Configuration Diff
...
- Signal: approved configuration snapshots and environment ownership produced 7 reviewable items in the selected scope.
...
- Review the evidence queue and confirm the owner for a redacted diff highlighting risky or unexplained changes.
- Keep the proposed output in draft state until the human reviewer approves the wording and scope.

## KPI snapshot

- configuration changes reviewed before rollout: baseline pending from the first two runs.
