# Illustrative Sample Output - Research Source Credibility Check
...
- Signal: research links, publication dates, and source claims produced 7 reviewable items in the selected scope.
...
- Review the evidence queue and confirm the owner for a source-quality note with corroboration status and freshness.
- Keep the proposed output in draft state until the human reviewer approves the wording and scope.

## KPI snapshot

- sources with a recorded verification status: baseline pending from the first two runs.
