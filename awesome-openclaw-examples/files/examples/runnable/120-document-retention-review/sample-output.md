# Illustrative Sample Output - Document Retention Review
...
- Signal: document metadata, retention rules, and exception notes produced 7 reviewable items in the selected scope.
...
- Review the evidence queue and confirm the owner for a review queue that separates candidates from records requiring counsel.
- Keep the proposed output in draft state until the human reviewer approves the wording and scope.

## KPI snapshot

- items reviewed before retention deadline: baseline pending from the first two runs.
