# Illustrative Sample Output - Voice of Customer Digest
...
- Signal: customer feedback across support, calls, and surveys produced 7 reviewable items in the selected scope.
...
- Review the evidence queue and confirm the owner for a theme digest with volume, representative evidence, and trend direction.
- Keep the proposed output in draft state until the human reviewer approves the wording and scope.

## KPI snapshot

- themes with a named product or service owner: baseline pending from the first two runs.
