# Illustrative Sample Output - Home Inventory Register
...
- Signal: household item notes, receipts, and warranty dates produced 7 reviewable items in the selected scope.
...
- Review the evidence queue and confirm the owner for an inventory register with evidence links and missing fields.
- Keep the proposed output in draft state until the human reviewer approves the wording and scope.

## KPI snapshot

- high-value items with a current record: baseline pending from the first two runs.
