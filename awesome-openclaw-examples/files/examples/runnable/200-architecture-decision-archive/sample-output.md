# Illustrative Sample Output - Architecture Decision Archive
...
- Signal: decision notes, linked code changes, and supersession signals produced 7 reviewable items in the selected scope.
...
- Review the evidence queue and confirm the owner for a dated decision record with context, tradeoffs, and review status.
- Keep the proposed output in draft state until the human reviewer approves the wording and scope.

## KPI snapshot

- decisions discoverable with a current status: baseline pending from the first two runs.
