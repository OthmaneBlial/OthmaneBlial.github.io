# Illustrative Sample Output - Data Dictionary Steward
...
- Signal: schema changes, field descriptions, and ownership notes produced 7 reviewable items in the selected scope.
...
- Review the evidence queue and confirm the owner for a proposed data-dictionary update with examples and deprecation flags.
- Keep the proposed output in draft state until the human reviewer approves the wording and scope.

## KPI snapshot

- time from schema change to documented field: baseline pending from the first two runs.
