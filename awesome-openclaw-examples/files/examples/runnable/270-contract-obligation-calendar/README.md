# 270 - Contract Obligation Calendar

Contract Obligation Calendar turns contract obligations, dates, owners, and source clauses into a proposed obligation calendar with review status. It is a bounded starter for weekly review, with human approval before any external write or outbound message.

## What It Does

- Collects contract obligations, dates, owners, and source clauses within the declared workflow scope.
- Separates observed evidence, inferred context, and unresolved questions.
- Produces a proposed obligation calendar with review status.
...
export CRON_NAME="Contract Obligation Calendar"
```

4. Check the local OpenClaw prerequisite:

```bash
bash examples/runnable/270-contract-obligation-calendar/scripts/check_prereqs.sh
```

5. Read [the illustrative sample output](sample-output.md), then install the draft-only cron job:

```bash
bash examples/runnable/270-contract-obligation-calendar/scripts/install_cron.sh
```
...
- obligations reviewed before their notice window.
...
- Treat contracts and commercial commitments as sensitive and minimize the source scope before the first run.
