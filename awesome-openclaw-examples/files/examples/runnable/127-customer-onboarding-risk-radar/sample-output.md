# Illustrative Sample Output - Customer Onboarding Risk Radar
...
- Signal: onboarding milestones, blockers, and customer communications produced 7 reviewable items in the selected scope.
...
- Review the evidence queue and confirm the owner for a risk-ranked onboarding view with the next unblocker.
- Keep the proposed output in draft state until the human reviewer approves the wording and scope.

## KPI snapshot

- accounts reaching first value on schedule: baseline pending from the first two runs.
