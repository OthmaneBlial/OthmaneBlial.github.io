# Illustrative Sample Output - Container Image Freshness
...
- Signal: image inventory, release dates, and approved freshness thresholds produced 7 reviewable items in the selected scope.
...
- Review the evidence queue and confirm the owner for a freshness digest with stale-image owners and evidence.
- Keep the proposed output in draft state until the human reviewer approves the wording and scope.

## KPI snapshot

- images reviewed before the freshness threshold: baseline pending from the first two runs.
