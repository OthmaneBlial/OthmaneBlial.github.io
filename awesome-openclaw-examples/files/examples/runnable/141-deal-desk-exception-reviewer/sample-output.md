# Illustrative Sample Output - Deal Desk Exception Reviewer
...
- Signal: non-standard commercial terms and approval policy produced 7 reviewable items in the selected scope.
...
- Review the evidence queue and confirm the owner for an exception brief with policy references and missing approvals.
- Keep the proposed output in draft state until the human reviewer approves the wording and scope.

## KPI snapshot

- exception requests returned complete: baseline pending from the first two runs.
