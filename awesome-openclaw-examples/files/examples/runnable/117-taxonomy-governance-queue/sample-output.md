# Illustrative Sample Output - Taxonomy Governance Queue
...
- Signal: new tags, category conflicts, and usage examples produced 7 reviewable items in the selected scope.
...
- Review the evidence queue and confirm the owner for a prioritized queue for taxonomy decisions with affected records.
- Keep the proposed output in draft state until the human reviewer approves the wording and scope.

## KPI snapshot

- taxonomy requests resolved within target: baseline pending from the first two runs.
