# Illustrative Sample Output - Content Brief Quality Gate
...
- Signal: content briefs, audience intent, and source requirements produced 7 reviewable items in the selected scope.
...
- Review the evidence queue and confirm the owner for a pre-draft quality check for scope, evidence, and a clear reader action.
- Keep the proposed output in draft state until the human reviewer approves the wording and scope.

## KPI snapshot

- briefs accepted without a second strategy pass: baseline pending from the first two runs.
