# Illustrative Sample Output - Health Appointment Prep
...
- Signal: appointment instructions, personal questions, and prior notes produced 7 reviewable items in the selected scope.
...
- Review the evidence queue and confirm the owner for a preparation checklist that never substitutes for clinical advice.
- Keep the proposed output in draft state until the human reviewer approves the wording and scope.

## KPI snapshot

- questions captured before the appointment: baseline pending from the first two runs.
