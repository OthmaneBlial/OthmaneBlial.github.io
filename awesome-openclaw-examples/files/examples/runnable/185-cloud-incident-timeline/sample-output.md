# Illustrative Sample Output - Cloud Incident Timeline
...
- Signal: incident events, alerts, and responder notes produced 7 reviewable items in the selected scope.
...
- Review the evidence queue and confirm the owner for a normalized timeline with source timestamps and missing intervals.
- Keep the proposed output in draft state until the human reviewer approves the wording and scope.

## KPI snapshot

- time to produce an incident timeline: baseline pending from the first two runs.
