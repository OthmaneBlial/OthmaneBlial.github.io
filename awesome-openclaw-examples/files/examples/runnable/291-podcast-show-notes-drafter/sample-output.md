# Illustrative Sample Output - Podcast Show Notes Drafter
...
- Signal: an episode transcript, guest links, and approved editorial angle produced 7 reviewable items in the selected scope.
...
- Review the evidence queue and confirm the owner for draft show notes with timestamps, links, and unanswered fact checks.
- Keep the proposed output in draft state until the human reviewer approves the wording and scope.

## KPI snapshot

- episodes with show notes ready for human review: baseline pending from the first two runs.
