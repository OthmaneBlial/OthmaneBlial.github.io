# Illustrative Sample Output - Podcast Guest Research Pack
...
- Signal: public guest sources and the planned conversation goal produced 7 reviewable items in the selected scope.
...
- Review the evidence queue and confirm the owner for a fact-checked briefing with questions, citations, and no invented biography.
- Keep the proposed output in draft state until the human reviewer approves the wording and scope.

## KPI snapshot

- guest prep time: baseline pending from the first two runs.
