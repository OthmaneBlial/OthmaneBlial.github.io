# Illustrative Sample Output - Backport Candidate Brief
...
- Signal: merged fixes, release branches, and risk labels produced 7 reviewable items in the selected scope.
...
- Review the evidence queue and confirm the owner for a backport candidate brief with compatibility evidence and reviewer ownership.
- Keep the proposed output in draft state until the human reviewer approves the wording and scope.

## KPI snapshot

- safe candidates reviewed before branch cut: baseline pending from the first two runs.
