# Illustrative Sample Output - Access Token Rotation Tracker
...
- Signal: token owner records, rotation dates, and approved systems produced 7 reviewable items in the selected scope.
...
- Review the evidence queue and confirm the owner for a rotation queue with no secret values and explicit escalation dates.
- Keep the proposed output in draft state until the human reviewer approves the wording and scope.

## KPI snapshot

- tokens rotated before their review date: baseline pending from the first two runs.
