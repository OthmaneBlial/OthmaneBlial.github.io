# Illustrative Sample Output - Meal Plan and Grocery Draft
...
- Signal: dietary constraints, planned meals, and pantry notes produced 7 reviewable items in the selected scope.
...
- Review the evidence queue and confirm the owner for a seven-day meal plan with a deduplicated draft grocery list.
- Keep the proposed output in draft state until the human reviewer approves the wording and scope.

## KPI snapshot

- planned meals completed without avoidable re-purchases: baseline pending from the first two runs.
