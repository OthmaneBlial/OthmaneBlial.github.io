# Illustrative Sample Output - Freelance Client Admin Digest
...
- Signal: client messages, deliverables, time notes, and due dates produced 7 reviewable items in the selected scope.
...
- Review the evidence queue and confirm the owner for a private digest of commitments, risks, and invoice follow-ups.
- Keep the proposed output in draft state until the human reviewer approves the wording and scope.

## KPI snapshot

- client commitments with an owner and due date: baseline pending from the first two runs.
