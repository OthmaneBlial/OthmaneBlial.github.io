# Illustrative Sample Output - Endpoint Patch Compliance Brief
...
- Signal: approved endpoint inventory and patch status produced 7 reviewable items in the selected scope.
...
- Review the evidence queue and confirm the owner for a compliance brief with stale devices and owner follow-up.
- Keep the proposed output in draft state until the human reviewer approves the wording and scope.

## KPI snapshot

- endpoints reviewed before the patch deadline: baseline pending from the first two runs.
