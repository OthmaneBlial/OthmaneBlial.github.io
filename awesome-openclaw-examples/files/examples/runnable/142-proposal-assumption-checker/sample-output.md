# Illustrative Sample Output - Proposal Assumption Checker
...
- Signal: proposal drafts, scope notes, and pricing assumptions produced 7 reviewable items in the selected scope.
...
- Review the evidence queue and confirm the owner for a pre-send check for contradictions, unsupported promises, and open questions.
- Keep the proposed output in draft state until the human reviewer approves the wording and scope.

## KPI snapshot

- proposal issues caught before customer send: baseline pending from the first two runs.
