# Illustrative Sample Output - Feature Adoption Nudge Planner
...
- Signal: feature usage gaps and eligible customer context produced 7 reviewable items in the selected scope.
...
- Review the evidence queue and confirm the owner for a draft-only outreach queue with reason, audience, and opt-out check.
- Keep the proposed output in draft state until the human reviewer approves the wording and scope.

## KPI snapshot

- eligible accounts receiving a reviewed nudge: baseline pending from the first two runs.
