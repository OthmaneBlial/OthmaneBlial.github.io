# Illustrative Sample Output - Transcript Quote Index
...
- Signal: a transcript and a declared topic or quote criterion produced 7 reviewable items in the selected scope.
...
- Review the evidence queue and confirm the owner for a timestamped quote index that preserves context and speaker uncertainty.
- Keep the proposed output in draft state until the human reviewer approves the wording and scope.

## KPI snapshot

- quotes verified against the source transcript: baseline pending from the first two runs.
