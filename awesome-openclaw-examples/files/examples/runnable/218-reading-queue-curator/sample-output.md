# Illustrative Sample Output - Reading Queue Curator
...
- Signal: saved links, reading goals, and available time produced 7 reviewable items in the selected scope.
...
- Review the evidence queue and confirm the owner for a ranked reading queue with why-now labels and source links.
- Keep the proposed output in draft state until the human reviewer approves the wording and scope.

## KPI snapshot

- articles completed that match the stated learning goal: baseline pending from the first two runs.
