# Illustrative Sample Output - Newsletter Archive Search
...
- Signal: past newsletter issues and a new editorial question produced 7 reviewable items in the selected scope.
...
- Review the evidence queue and confirm the owner for a source-linked archive digest with relevant passages and dates.
- Keep the proposed output in draft state until the human reviewer approves the wording and scope.

## KPI snapshot

- time to retrieve a prior published reference: baseline pending from the first two runs.
