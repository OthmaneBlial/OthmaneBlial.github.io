# Illustrative Sample Output - Lecture Notes to Revision Pack
...
- Signal: a lecture recording or transcript and learning objectives produced 7 reviewable items in the selected scope.
...
- Review the evidence queue and confirm the owner for a revision pack with key ideas, open questions, and source timestamps.
- Keep the proposed output in draft state until the human reviewer approves the wording and scope.

## KPI snapshot

- revision packs reviewed before exam preparation: baseline pending from the first two runs.
