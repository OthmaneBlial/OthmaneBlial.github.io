# Illustrative Sample Output - Museum Visit Learning Guide
...
- Signal: public exhibit information and a learner's declared interest produced 7 reviewable items in the selected scope.
...
- Review the evidence queue and confirm the owner for a source-linked visit guide with questions and accessibility notes.
- Keep the proposed output in draft state until the human reviewer approves the wording and scope.

## KPI snapshot

- visit plans with verified exhibit details: baseline pending from the first two runs.
