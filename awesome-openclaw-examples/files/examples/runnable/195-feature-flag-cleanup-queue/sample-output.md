# Illustrative Sample Output - Feature Flag Cleanup Queue
...
- Signal: feature flags, owners, expiry dates, and code references produced 7 reviewable items in the selected scope.
...
- Review the evidence queue and confirm the owner for a cleanup queue that keeps removal proposals behind review.
- Keep the proposed output in draft state until the human reviewer approves the wording and scope.

## KPI snapshot

- expired flags with an assigned next action: baseline pending from the first two runs.
