# Illustrative Sample Output - Personal Knowledge Inbox
...
- Signal: saved notes, links, voice memos, and loose ideas produced 7 reviewable items in the selected scope.
...
- Review the evidence queue and confirm the owner for a triage queue that proposes tags and a next review destination.
- Keep the proposed output in draft state until the human reviewer approves the wording and scope.

## KPI snapshot

- inbox items processed without losing source context: baseline pending from the first two runs.
