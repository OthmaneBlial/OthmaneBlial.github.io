# Illustrative Sample Output - Metric Anomaly Narrator
...
- Signal: metric changes that cross a declared threshold produced 7 reviewable items in the selected scope.
...
- Review the evidence queue and confirm the owner for an evidence-first narrative separating signal, likely causes, and unknowns.
- Keep the proposed output in draft state until the human reviewer approves the wording and scope.

## KPI snapshot

- alert-to-explanation time: baseline pending from the first two runs.
