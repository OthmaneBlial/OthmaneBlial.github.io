# Illustrative Sample Output - Change Management FAQ
...
- Signal: change plan, employee questions, and approved policy language produced 7 reviewable items in the selected scope.
...
- Review the evidence queue and confirm the owner for a draft FAQ that labels policy decisions still awaiting approval.
- Keep the proposed output in draft state until the human reviewer approves the wording and scope.

## KPI snapshot

- repeat change questions answered from approved material: baseline pending from the first two runs.
