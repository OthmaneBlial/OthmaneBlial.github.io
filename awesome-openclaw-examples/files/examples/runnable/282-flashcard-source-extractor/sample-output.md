# Illustrative Sample Output - Flashcard Source Extractor
...
- Signal: a source document and a declared learning objective produced 7 reviewable items in the selected scope.
...
- Review the evidence queue and confirm the owner for draft flashcards with page or section references and confidence labels.
- Keep the proposed output in draft state until the human reviewer approves the wording and scope.

## KPI snapshot

- cards reviewed before entering the study set: baseline pending from the first two runs.
