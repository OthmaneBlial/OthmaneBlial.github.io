...
- Hardened the API terminal polling smoke so a worker is considered finished
  ...
  zero failures and two inherited ignored tests; the current source adds two claim-link checks, and the targeted evidence-model suite passes 7 tests per binary.
...
- Hardened evidence persistence. Every evidence record now requires complete
  ...
  limitations, confidence notes and support/challenge claims before a hypothesis can be persisted, with at least one non-empty linked claim required.
...
- Localized SEO, page-quality, accessibility, static-render and browser-timing rubric
  labels and dimension names across all six dashboard locale catalogs. Known
  evaluator-generated observations now map their counts, HTML states, viewport
  checks and timing tokens through the same catalogs; unrecognized evaluator
  text and user-authored run content remain explicit source-language follow-up
  work.
...
passed, zero failures and two inherited ignored tests**; the current source adds claim-link validation and its targeted evidence-model suite passes **7 tests per binary** with zero failures, including incomplete, credential-shaped, claim and confidence-rationale rejection coverage;
...
Distribution target matrix (2026-09-17): **corrected locally**. `dist-workspace.toml` now includes the published `x86_64-pc-windows-gnu` target and leaves MSVC out until a Windows SDK runner can build and validate it. Regenerated PowerShell and shell templates select the GNU archive when that is the available Windows artifact; `scripts/test-installer-templates.py --check-shell` now checks all five published shell targets (macOS arm64/x86_64, Linux musl arm64/x86_64 and Windows GNU), while the PowerShell template is checked against the Windows GNU asset. Shell syntax and the PowerShell parser passed. GitHub Actions remains manually disabled.
