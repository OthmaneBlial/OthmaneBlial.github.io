...
- Hardened the API terminal polling smoke so a worker is considered finished
  ...
  targeted runs and the full 970-test Rust target suite passes with 968 passed,
...
- Hardened evidence persistence. Every evidence record now requires complete
  ...
  limitations, confidence notes and support/challenge claims before a hypothesis can be persisted.
...
`cargo test --release --locked -- --test-threads=1` passes **970 tests per binary, 968
passed, zero failures and two inherited ignored tests** on the current source, including incomplete and credential-shaped evidence, claim and confidence-rationale rejection coverage;
